"""Persistent Python REPL Session Engine & Output Budgeting for Arthur."""
...
from arthur.browser import Browser, browser as default_browser
from arthur.errors import ArthurError

logger = logging.getLogger("arthur.repl")
...
class ExecutionTimeoutContext:
    """Watchdog execution timer for code execution."""

    def __init__(self, timeout: Optional[float] = 30.0):
        self.timeout = timeout
        self._old_handler: Any = None

    def __enter__(self) -> "ExecutionTimeoutContext":
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        if (
            hasattr(signal, "SIGALRM")
            and threading.current_thread() is threading.main_thread()
            and self._old_handler is not None
        ):
            try:
...
class OutputBudgetFormatter:
    ...
    def format_execution_result(
        ...
        """Format outputs into bounded blocks."""
        sections: List[str] = []
        ...
        if auto_snapshot and auto_snapshot.strip():
            ...
            truncated_snapshot = self._truncate_string(
                auto_snapshot.strip(), snapshot_budget
            )
        ...
        raw_output = "\n\n".join(sections)
        return str(defang_telemetry_payload(raw_output))
    ...
    def _serialize_value(
        ...
        if isinstance(val, str):
            if len(val) > budget or (
                len(val) > self.string_head_tail and current_depth > 0
            ):
            ...
            for item in items[: self.max_items]:
                item_strs.append(
                    self._serialize_value(
                        ...
                        budget=budget // min(len(items), self.max_items),
                    )
                )
        ...
        if isinstance(val, dict):
            ...
            for k in keys[: self.max_items]:
                ...
                v_str = self._serialize_value(
                    val[k],
                    current_depth=current_depth + 1,
                    budget=budget // min(len(keys), self.max_items),
                )
...
class PythonReplSession:
    ...
    def __init__(
        ...
        self.browser = browser_instance or default_browser
    ...
    def execute(self, code: str, timeout: Optional[float] = 30.0) -> str:
        ...
        # Check if trailing node is an expression statement
        ...
        try:
                ...
                with contextlib.redirect_stdout(stdout_buf), contextlib.redirect_stderr(
                    ...
                    if last_is_expr:
                        ...
                        if stmt_nodes:
                            ...
                            stmt_code = compile(
                                stmt_module, filename="<repl>", mode="exec"
                            )
            ...
            elif isinstance(e, ArthurError) and hasattr(self.browser, "snapshot"):
                try:
                    auto_snapshot = self.browser.snapshot()
                except Exception:
                    pass
    ...
    def _sanitize_traceback(self, exc: BaseException) -> str:
        ...
        for frame in extracted:
            ...
            if fn == "<repl>":
                ...
                if not frame.name.startswith("_") and frame.name not in (
                    "call",
                    "connect",
                    "close",
                ):
    ...
    def reset(self) -> None:
        """Reset session globals while retaining browser instance."""
        self._globals = {
            "__name__": "__main__",
            "__doc__": None,
            "__builtins__": builtins,
